...
#
# Usage: ABCEXTERNAL=/path/to/abc bash tests/techmap/abc_src_retention_full.sh
...
# Run synthesis with abc -lut
$YOSYS -p "
read_verilog /tmp/test_src_ret.v
synth -top counter -flatten
abc -lut 4
write_json /tmp/test_src_ret_output.json
" 2>&1

# Check if any cells have src attributes
SRC_CELLS=$(python3 -c "
import json
with open('/tmp/test_src_ret_output.json') as f:
    data = json.load(f)
count = 0
...
# Cleanup
rm -f /tmp/test_src_ret.v /tmp/test_src_ret_output.json
